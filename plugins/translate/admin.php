...
// 保存 API 配置
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['translate_action'] ?? '') === 'save_api') {
...
// API 翻译单条
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['translate_action'] ?? '') === 'translate_one') {
    $text = $_POST['text'] ?? '';
    $result = _pluginTranslate($text, $targetLang, $settings);
    if ($result !== false) {
...
// API 批量翻译
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['translate_action'] ?? '') === 'translate_batch') {
    $untranslated = [];
    ...
    foreach ($untranslated as $key => $value) {
        $result = _pluginTranslate($value, $targetLang, $settings);
        if ($result !== false) {
    ...
    _pluginSaveLangFile($targetLangFile, $targetLang, $targetData);
    success(['count' => $translated], "已翻译 {$translated} 条");
}

// 手动保存翻译
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['translate_action'] ?? '') === 'save') {
...
function _pluginTranslate(string $text, string $targetLang, $settings): string|false
{
    ...
    if ($api === 'deepl') {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => http_build_query(['text' => $text, 'source_lang' => 'ZH', 'target_lang' => $target]),
            CURLOPT_HTTPHEADER => ['Authorization: DeepL-Auth-Key ' . $apiKey, 'Content-Type: application/x-www-form-urlencoded'],
            CURLOPT_TIMEOUT => 15,
        ]);
        ...
        $url = 'https://translation.googleapis.com/language/translate/v2?' . http_build_query([
            'key' => $apiKey, 'q' => $text, 'source' => 'zh-CN', 'target' => $target, 'format' => 'text',
        ]);
...
function _pluginSaveLangFile(string $file, string $lang, array $data): void
{
...
<div class="bg-white rounded-lg shadow-sm p-4 mb-6 flex flex-wrap items-center justify-between gap-3">
    <div class="flex items-center gap-3">
        ...
        <select onchange="location.href='?plugin=translate&lang='+this.value+'&filter=<?php echo $filter; ?>'" class="border rounded px-3 py-1.5 text-sm">
        ...
        <div class="flex border rounded overflow-hidden text-sm">
            <a href="?plugin=translate&lang=<?php echo $targetLang; ?>&filter=all" class="px-3 py-1.5 <?php echo $filter === 'all' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-50'; ?>">全部</a>
            <a href="?plugin=translate&lang=<?php echo $targetLang; ?>&filter=untranslated" class="px-3 py-1.5 border-l <?php echo $filter === 'untranslated' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-50'; ?>">未翻译</a>
            <a href="?plugin=translate&lang=<?php echo $targetLang; ?>&filter=translated" class="px-3 py-1.5 border-l <?php echo $filter === 'translated' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-50'; ?>">已翻译</a>
        </div>
    </div>
    ...
    <div class="flex items-center gap-2">
        ...
        <button onclick="saveAll()" class="bg-primary hover:bg-secondary text-white px-4 py-1.5 rounded text-sm">保存</button>
    </div>
</div>

<!-- 翻译列表 -->
<form id="translateForm">
    <input type="hidden" name="translate_action" value="save">
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 w-20">操作</th>
                </tr>
            </thead>
            <tbody class="divide-y">
                <?php foreach ($sourceData as $key => $value):
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 text-center">
                        <button type="button" onclick="translateOne(this, '<?php echo e(addslashes($value)); ?>')"
...
function saveApiConfig() {
    var fd = new FormData();
    fd.append('translate_action', 'save_api');
    fd.append('api', document.getElementById('apiProvider').value);
    fd.append('api_key', document.getElementById('apiKey').value);
    fetch('', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => { d.code === 0 ? showMessage(d.msg) : showMessage(d.msg, 'error'); });
}

function translateOne(btn, text) {
    ...
    var fd = new FormData();
    fd.append('translate_action', 'translate_one');
    fd.append('text', text);
    fetch('', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => {
            ...
            } else showMessage(d.msg || '翻译失败', 'error');
        })
        .finally(() => { btn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"></path></svg>'; });
}
...
function batchTranslate() {
    ...
    var fd = new FormData();
    fd.append('translate_action', 'translate_batch');
    fetch('', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => { if (d.code === 0) { showMessage(d.msg); setTimeout(() => location.reload(), 1500); } else showMessage(d.msg, 'error'); })
        .finally(() => { btn.disabled = false; btn.textContent = 'API一键翻译'; });
}

...
// 言語パック（デフォルト: 日本語）
$lang = $_GET['lang'] ?? 'ja';
$langFile = INSTALL_PATH . "/lang/{$lang}.php";
if (!file_exists($langFile)) { $lang = 'ja'; $langFile = INSTALL_PATH . "/lang/ja.php"; }
$L = require $langFile;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'install') {
        try {
            ...
            $siteName = $_POST['site_name'] ?? 'ikaiCMS';
            ...
            <?php elseif ($step === 3): ?>
                ...
                <form id="adminForm" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="site_name" value="ikaiCMS" class="w-full border rounded px-3 py-2" required>
                    </div>
                    <div>
                ...
                <div class="text-center">
                    ...
                    <div class="bg-white border rounded-lg text-left mb-6">
                        <div class="px-5 py-3 border-b bg-gray-50 rounded-t-lg">
                            <h3 class="font-bold text-gray-800 text-sm"><?php echo $L['rewrite_title'] ?? 'URLリライト設定'; ?></h3>
                            <p class="text-xs text-gray-500 mt-1"><?php echo $L['rewrite_desc'] ?? 'きれいなURL（/company.html 等）を使うにはWebサーバーの設定が必要です'; ?></p>
                        </div>
                        <div class="p-5">
                            ...
                            <div id="panel-apache">
                                <div style="background:#f0fdf4;color:#15803d;padding:12px;border-radius:6px;font-size:14px">
                                    <strong>&#10003; <?php echo $L['rewrite_apache_ok'] ?? '設定不要'; ?></strong><br>
                                    <?php echo $L['rewrite_apache_desc'] ?? '.htaccess ファイルが同梱されています。mod_rewrite が有効であれば自動的に動作します。'; ?>
                                </div>
                                <div style="margin-top:8px;font-size:12px;color:#6b7280">
                                    <?php echo $L['rewrite_apache_check'] ?? 'Apache の httpd.conf で AllowOverride All が設定されていることを確認してください。'; ?>
                                </div>
                            </div>
                            <div id="panel-nginx" style="display:none">
                                <div style="background:#fff7ed;color:#c2410c;padding:12px;border-radius:6px;font-size:14px;margin-bottom:8px">
                                    <strong>&#9888; <?php echo $L['rewrite_nginx_manual'] ?? '手動設定が必要'; ?></strong><br>
                                    <?php echo $L['rewrite_nginx_desc'] ?? '以下の内容をサイトの Nginx 設定ファイル（server ブロック内）に追加してください。'; ?>
                                </div>
...
location / { try_files $uri $uri/ /index.php?$query_string; }</code></pre>
                                    <button onclick="copyNginxCode(this)" style="position:absolute;top:8px;right:8px;background:#334155;color:#fff;border:none;padding:4px 12px;border-radius:4px;font-size:12px;cursor:pointer">Copy</button>
                                </div>
                                <div style="margin-top:8px;font-size:12px;color:#6b7280">
                                    <?php echo $L['rewrite_nginx_reload'] ?? '設定後、nginx -t で構文チェックし、nginx -s reload で反映してください。'; ?>
                                </div>
                            </div>
                            <script>
                            ...
                            function copyNginxCode(btn) {
                                var code = document.getElementById('nginxCode').querySelector('code');
                                ...
                                if (done) {
                                    ...
                                    setTimeout(function() { btn.textContent = 'Copy'; btn.style.background = '#334155'; }, 2000);
                                } else {
                                    btn.textContent = '選択済み Ctrl+C';
                                    btn.style.background = '#d97706';
                                    setTimeout(function() { btn.textContent = 'Copy'; btn.style.background = '#334155'; }, 4000);
                                }
                            }
                            </script>
                        </div>
        ...
        <div class="text-center text-gray-400 text-sm mt-8">
            &copy; <?php echo date('Y'); ?> <a href="https://www.ikaicms.com" target="_blank" class="hover:text-gray-600 transition">ikaiCMS</a>
        </div>
    </div>
</body>
</html>
